...
void GraphicDesigner::show_worms(StageDTO s, SDL_Surface *screen, SDL_Rect camera_position){
  for (auto w: s.worms) {
    ...
    oLog() << "[GraphicDesigner] last turn " << this->last_worm_turn
          << ", this turn " << s.worm_turn << "\n";
    ...
    if(worm_info.player_id > 4){
      oLog() << "Error: juego no preparado para mas de 4 jugadores\n";
    }
...
bool GraphicDesigner::is_inside_weapon_menu(int x, int y){
  ...
  oLog() << "[GraphicDesigner] se hizo click en el menu\n";
  return true;
}
